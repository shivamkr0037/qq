...
def quiz(update, context):
    ...
    for question in questions:
        ...
        probable_categories = computeCategories([category], user_response_NJ)
        probable_choices = computeChoices(user_response, choices)
        if len(probable_categories) > 0:
            response = "\n".join([response, "You seem to be talking about '{}'!".format(probable_categories[category])])
        elif len(probable_choices) > 0:
            response = "\n".join([response, "You seem to be talking about the possible choices {}!".format(' '.join(probable_choices))])
        else:
            response = "\n".join([response, "I'm not sure what you're talking about, please try again!"])
            continue
        if probable_categories.get(category) == question['category']:
            score, response = displayBotResponse(score, correct_answer, True)
        else:
            score, response = displayBotResponse(score, wrong_answer, False, question['correct_answer'])
        context.bot.send_message(chat_id=update.effective_chat.id, text=response)
    context.bot.send_message(chat_id=update.effective_chat.id, text="Quiz Over! Final Score = {}".format(score))
